...
def _truncate_long_urls(text: str, max_url_length: int = 100) -> str:
    """Truncate long URLs in text for better display."""
    import re
    ...
    def truncate_url(match):
        ...
        if len(url) > max_url_length:
            return url[:max_url_length-3] + "..."
        return url
    
    return re.sub(url_pattern, truncate_url, text, flags=re.IGNORECASE)
...
def _format_agent_output(model_output: AgentOutput) -> str:
    ...
    if model_output:
        try:
            ...
            # Truncate long URLs in the JSON output for better display
            json_string = _truncate_long_urls(json_string)
...
PLACEHOLDERS["PLACEHOLDER_URL"] = response["AuthorizedUrl"]""",
        ...
        user_input = gr.Textbox(
            ...
            value="open PLACEHOLDER_URL and open space names test1234 else create it",
            info="Describe what you want the agent to do",
            max_lines=10,  # Allow expansion for long URLs
        )
